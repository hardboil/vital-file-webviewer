...
/**
 * Draw event markers on waveform
 * @param {CanvasRenderingContext2D} ctx - Canvas context
 * @param {Object} vitalFile - Vital file data
 ...
 */
export function drawEventMarkers(ctx, vitalFile, x, y, width, height, startTime, endTime) {
  const evtTrack = vitalFile.eventTrack;
  if (!evtTrack || !evtTrack.data || evtTrack.data.length === 0) return;
  ...
  for (const [dt, value] of evtTrack.data) {
    if (dt < startTime || dt > endTime) continue;

    const px = x + ((dt - startTime) / duration) * width;

    // Draw event marker line
    ctx.strokeStyle = '#4EB8C9';
    ctx.lineWidth = 1;
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    ctx.moveTo(px, y);
    ...
    const label = value.length > 15 ? value.substring(0, 15) + '...' : value;
    ctx.fillText(label, px + 2, y + 12);
  }
